...
public abstract class FaunaClient {
                ...
                response -> {
                    ...
                    if (response.statusCode() >= 400) {
                        ...
                        QueryFailure failure =
                                new QueryFailure(response.statusCode(),
                                        QueryResponse.builder(codec));
                        ErrorHandler.handleQueryFailure(response.statusCode(),
                                failure);
                        // Fall back on ProtocolException.
                        throw new ProtocolException(response.statusCode(),
                                failure);
                    }
    ...
    public <E> CompletableFuture<FaunaStream<E>> asyncStream(
            StreamRequest streamRequest, Class<E> elementClass) {
        HttpRequest streamReq =
                getStreamRequestBuilder().buildStreamRequest(streamRequest);
    ...
    public <E> FaunaStream<E> stream(StreamRequest streamRequest,
        ...
        return completeAsync(asyncStream(streamRequest, elementClass),
                STREAM_SUBSCRIPTION);
    }
    ...
    public <E> CompletableFuture<FaunaStream<E>> asyncStream(Query fql,
        ...
        return this.asyncQuery(fql, EventSourceResponse.class).thenApply(
                queryResponse -> this.stream(StreamRequest.fromTokenResponse(
                        queryResponse.getData()), elementClass));
    }
     ...
     */
    public <E> CompletableFuture<FeedPage<E>> poll(EventSource source,
                                                   FeedOptions feedOptions,
                                                   Class<E> elementClass) {
        return new RetryHandler<FeedPage<E>>(getRetryStrategy(),
                logger).execute(makeAsyncFeedRequest(
                getHttpClient(),
                getFeedRequestBuilder().buildFeedRequest(source,
