...
public class ClientResponseException extends ClientException {
    ...
    private static String buildMessage(String message, int statusCode) {
        return MessageFormat.format("ClientResponseException HTTP {0}: {1}", statusCode, message);
    }

    public ClientResponseException(String message, Throwable exc, int statusCode) {
