...
public class MetricsHandler {
    private static final String unique =
            System.getenv("LOG_UNIQUE") != null ? System.getenv("LOG_UNIQUE") :
                    "";
    private static final String rawStatsFilename =
            "rawstats_" + unique + ".csv";
    private static final String statsBlockFilename = "stats_" + unique + ".txt";
    private static final Map<String, List<TestTimings>> metricsCollector =
            new HashMap<>();

    public static void recordMetrics(String queryName, int roundTripMs,
                                     int queryTimeMs) {
    ...
    public static void writeMetricsToFile() throws IOException {
        List<String> headers =
                List.of("ts,metric,roundTrip,queryTime,diff,tags");
        Files.write(Paths.get(rawStatsFilename), headers,
                StandardOpenOption.CREATE);

        List<String> blockHeaders = List.of(
                String.format("%-35s%9s%9s%9s%9s", "TEST", "P50", "P95", "P99",
                        "STDDEV"),
                new String(new char[71]).replace("\0", "-")
        );
        Files.write(Paths.get(statsBlockFilename), blockHeaders,
                StandardOpenOption.CREATE);

        for (Map.Entry<String, List<TestTimings>> entry : new TreeMap<>(
                metricsCollector).entrySet()) {
            ...
            Files.write(Paths.get(rawStatsFilename), lines,
            ...
            double stddev =
                    Math.round(stats.getStandardDeviation() * 100) / 100;

            var line =
                    String.format("%-35s%9s%9s%9s%9s", entry.getKey(), p50, p95,
                            p99, stddev);
            Files.write(Paths.get(statsBlockFilename), List.of(line),
                    StandardOpenOption.CREATE, StandardOpenOption.APPEND);
        }
    }

    public static String[] getMetricsTags() {
        String env = System.getenv("FAUNA_ENVIRONMENT") != null ?
                System.getenv("FAUNA_ENVIRONMENT") : "test";
