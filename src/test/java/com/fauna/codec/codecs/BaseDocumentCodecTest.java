...
public class BaseDocumentCodecTest extends TestBase {
    public static final Codec<BaseDocument> BASE_DOCUMENT_CODEC =
            DefaultCodecProvider.SINGLETON.get(BaseDocument.class);
    ...
    public static Stream<Arguments> testCases() {
        return Stream.of(
                Arguments.of(TestType.Decode, BASE_DOCUMENT_CODEC,
                        DOCUMENT_WIRE, DOCUMENT, null),
                Arguments.of(TestType.Decode, BASE_DOCUMENT_CODEC,
                        NAMED_DOCUMENT_WIRE, NAMED_DOCUMENT, null),
                Arguments.of(TestType.Decode, BASE_DOCUMENT_CODEC,
                        NULL_DOC_WIRE, null, NULL_DOC_EXCEPTION),
                Arguments.of(TestType.Decode, BASE_DOCUMENT_CODEC,
                        DOCUMENT_REF_WIRE, null, new CodecException(
                                "Unexpected type `class com.fauna.types.DocumentRef` decoding with `BaseDocumentCodec<BaseDocument>`")),
                Arguments.of(TestType.Encode, BASE_DOCUMENT_CODEC,
                        DOCUMENT_REF_WIRE, DOCUMENT, null),
                Arguments.of(TestType.Encode, BASE_DOCUMENT_CODEC,
                        NAMED_DOCUMENT_REF_WIRE, NAMED_DOCUMENT, null)
        );
    }

    @ParameterizedTest(name = "BaseDocumentCodec({index}) -> {0}:{1}:{2}:{3}:{4}")
    @MethodSource("testCases")
    public <T, E extends Exception> void baseDoc_runTestCases(TestType testType,
                                                              Codec<T> codec,
                                                              String wire,
                                                              Object obj,
                                                              E exception)
    ...
    public void baseDoc_runUnsupportedTypeTestCases(String wire, FaunaType type)
            throws IOException {
        var exMsg = MessageFormat.format(
                ...
                type);
        runCase(TestType.Decode, BASE_DOCUMENT_CODEC, wire, null,
