...
public class NullableDocumentCodecTest extends TestBase {
    public static final Codec<NullableDocument<Document>> NULLABLE_DOC_CODEC =
            (Codec) DefaultCodecProvider.SINGLETON.get(NullableDocument.class,
                    new Type[] {Document.class});
    public static final Codec<NullableDocument<ClassWithAttributes>>
            NULLABLE_ClASS_CODEC =
            (Codec) DefaultCodecProvider.SINGLETON.get(NullableDocument.class,
                    new Type[] {ClassWithAttributes.class});
    ...
    public static final NullDocument<Document> NULL_DOCUMENT =
            new NullDocument<>("123", new Module("Foo"), "not found");

    public static Stream<Arguments> testCases() {
        return Stream.of(
                Arguments.of(TestType.Decode, NULLABLE_DOC_CODEC, DOCUMENT_WIRE,
                        new NonNullDocument<>(DOCUMENT), null),
                Arguments.of(TestType.Encode, NULLABLE_DOC_CODEC,
                        DOCUMENT_REF_WIRE, new NonNullDocument<>(DOCUMENT),
                        null),
                Arguments.of(TestType.Decode, NULLABLE_DOC_CODEC,
                        NAMED_DOCUMENT_WIRE,
                        new NonNullDocument<>(NAMED_DOCUMENT), null),
                Arguments.of(TestType.Encode, NULLABLE_DOC_CODEC,
                        NAMED_DOCUMENT_REF_WIRE,
                        new NonNullDocument<>(NAMED_DOCUMENT), null),
                Arguments.of(TestType.Decode, NULLABLE_DOC_CODEC, NULL_DOC_WIRE,
                        NULL_DOCUMENT, null),
                Arguments.of(TestType.Decode, NULLABLE_ClASS_CODEC,
                        DOCUMENT_WIRE,
                        new NonNullDocument<>(CLASS_WITH_ATTRIBUTES), null),
                Arguments.of(TestType.Encode, NULLABLE_ClASS_CODEC,
                        CLASS_WITH_ATTRIBUTES_WIRE,
                        new NonNullDocument<>(CLASS_WITH_ATTRIBUTES), null),
                Arguments.of(TestType.Decode, NULLABLE_ClASS_CODEC,
                        NULL_DOC_WIRE, NULL_DOCUMENT, null)
        );
    }

    @ParameterizedTest(name = "NullableCodec({index}) -> {0}:{1}:{2}:{3}:{4}")
    @MethodSource("testCases")
    public <T, E extends Exception> void nullable_runTestCases(
    ...
    public void nullable_runUnsupportedTypeTestCases(String wire,
                                                     FaunaType type)
            throws IOException {
        // This codec will pass through the supported types of the underlying codec
        var exMsg = MessageFormat.format(
                "Unable to decode `{0}` with `BaseDocumentCodec<BaseDocument>`. Supported types for codec are [Document, Null, Ref].",
                type);
        runCase(TestType.Decode, NULLABLE_DOC_CODEC, wire, null,
