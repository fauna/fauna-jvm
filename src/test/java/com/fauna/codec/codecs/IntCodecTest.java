...
public class IntCodecTest extends TestBase {
    ...
    public void int_runUnsupportedTypeTestCases(String wire, FaunaType type) throws IOException {
        var exMsg = MessageFormat.format("Unable to decode `{0}` with `IntCodec<Integer>`. Supported types for codec are [Int, Long, Null].", type);
        runCase(TestType.Decode, INT_CODEC, wire, null, new ClientException(exMsg));
    }
}
