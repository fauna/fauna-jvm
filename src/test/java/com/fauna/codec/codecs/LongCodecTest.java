...
public class LongCodecTest extends TestBase {
    ...
    public void long_runUnsupportedTypeTestCases(String wire, FaunaType type) throws IOException {
        var exMsg = MessageFormat.format("Unable to decode `{0}` with `LongCodec<Long>`. Supported types for codec are [Int, Long, Null].", type);
        runCase(TestType.Decode, LONG_CODEC, wire, null, new ClientException(exMsg));
    }
}
