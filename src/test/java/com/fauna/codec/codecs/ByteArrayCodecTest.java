...
public class ByteArrayCodecTest extends TestBase {
    ...
    public void byteArray_runUnsupportedTypeTestCases(String wire, FaunaType type) throws IOException {
        var exMsg = MessageFormat.format("Unable to decode `{0}` with `ByteArrayCodec<byte[]>`. Supported types for codec are [Bytes, Null].", type);
        runCase(TestType.Decode, BYTE_ARRAY_CODEC, wire, null, new ClientException(exMsg));
    }
}
